...
mkOptions opts@Options {..} = do
    ...
    let depreceatedPkgDb = baseDir </> "package-database" </> "depreceated"
    hasDefaultPkgDb <- Dir.doesDirectoryExist defaultPkgDb
    hasDepreceatedPkgDb <- Dir.doesDirectoryExist depreceatedPkgDb
    let pkgDb
            | hasDepreceatedPkgDb = [depreceatedPkgDb]
